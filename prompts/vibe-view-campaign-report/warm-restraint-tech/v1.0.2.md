...
:root {
  ...
  --ember-wash:    oklch(0.72 0.16 50 / 0.12);
  
}
```

Color rules:
- Focal numbers use `--primary` (L 0.72 C 0.16). Units, unit suffixes, percent signs, delta arrows, and delta values use `--primary-hl` (same hue, lighter L 0.78 C 0.14).
...
Brand hue: `--brand-hue` is the only accent knob. Accent tokens take hue from it; neutral tokens (backgrounds, surfaces, foregrounds, borders) fixed at hue 80.
...
```css
/* npm loading — primary Noto Sans SC Variable · display Geist Variable · mono Geist Mono */
```

Font stacks: Primary Sans `'Noto Sans SC Variable', 'Noto Sans SC', 'PingFang SC', 'Helvetica Neue', sans-serif`; Display Number `'Geist Variable', 'Geist', 'Noto Sans SC', sans-serif`; Mono Meta `'Geist Mono', ui-monospace, monospace`. 

Weight: `font-normal` (400) prose · `font-medium` (500) titles, Display Number, delta value, chapter marker, unit suffix. No `font-semibold`/`font-bold`/≥600 — restraint is the signature.

| Level | lg | mobile | Weight |
|-------|----|--------|--------|
| Hero Title | `text-[88px]` | `text-[44px]` | `font-medium` |
| Display Number | `text-[140px]` | `text-[64px]` | `font-medium` |
| Section Primary | `text-[56px]` | — | `font-medium` |
| Section Secondary | `text-[40px]` | — | `font-medium` |
| Section Tertiary | `text-[24px]` | — | `font-normal` |
| Quote | `text-[32px]` | — | `font-normal` |
| Lead Paragraph | `text-[18px]` | — | `font-normal` |
| Body | `text-[15px]` | — | `font-normal` |
| Caption | `text-[13px]` | — | `font-normal` |
| Unit Suffix | `text-[40px]` | — | `font-medium` |
| Meta/Label | `text-[11px]` | — | `font-medium` `tracking-[0.12em]` |
| Eyebrow | `text-[11px]` | — | `font-medium` `tracking-[0.16em]` |

Display Number `leading-[0.92]` `tracking-[-0.05em]`. Focal numbers use exact breakpoint classes (no `clamp()` — sub-pixel jitter degrades spring count-up). Floor `text-[10px]`; required reading `text-[14px]+` per §8.
...
OpenType: `font-feature-settings: "kern", "tnum", "lnum", "ss01"` on all data numbers.

CJK: primary sans for Hero Title, Chapter Title, Body, Caption when Chinese; display stack for Display Number (Latin only); `textWrap: "balance"`; `max-w-[52ch]` body / `max-w-[20ch]` hero; 0.1em space flanking digits in mixed lines.
...
Hierarchy: one Primary number per section (largest, `--primary`, `--primary-hl` on unit/delta). Tiers differ ≥ 2 steps. Emphasis from scale and color — never italic, never bold above 500.
...
Radius: pill/tag: rounded-full. Sharp panel (table/chart): `rounded-[0px]`–`rounded-[4px]`. Card chrome (cluster/commitments): `rounded-[16px]`.
...
Shader policy: 1 WebGL instance per page (Hero only). Pauses to `speed: 0` off-viewport, returns `speed: 1.72` on re-enter.

Brand wash: `--ember-wash` ~12–18% alpha radial gradient marks Hero focal value and Outro signature only. Above shader, below text.



Chapter opener: `ChapterBanner` — `--border-strong` hairline + mono accent kicker + chapter title `--foreground` sans medium. Left-aligned, no gradient band.

Chapter divider: 1px `--border` + `· · ·` `--foreground-3` Mono `text-[13px]` `tracking-[0.8em]`. In-chapter rest only.

Panel discipline: `rounded-[2px]`–`rounded-[4px]` chart frames, `rounded-[16px]` card chrome. Each archetype one material logic.
...
| WebGL Hero | animated, viewport-paused | speed: 1.72 |
...
**Every `whileInView` in the entire page must pair with `viewport={{ once: true, margin: "0px 0px -10% 0px" }}`** — no exceptions. Without `once: true`, elements reset to initial (opacity 0 / scaleX 0) on scroll-out and disappear.
...
Continuous motion: Hero `GrainGradient` speed 1.72 in viewport, 0 out. SVG strokes one-shot. No infinite breathing, no scroll-linked transforms.

## 7. Iconography

`lucide-react`, 1px stroke, outline-only, `currentColor`, rem sizing, `gap-2` to text. Icons ≤ cap-height. Decorative: `aria-hidden`. Semantic: `role="img"` + `aria-label`. Custom SVG only for structural-only use cases. Unicode ▲ ▼ · — • allowed inline.
...
- **Don't** use `whileInView` without `viewport={{ once: true, margin: "0px 0px -10% 0px" }}`.
...
Copy register: 象牙暖光杂志风,暖橙呼吸 ember. Chapter titles 5–14 CJK / 4–9 EN words, `font-medium` sans. Eyebrow tracking `0.16em`. Resist slogans, questions, ALL-CAPS.
...
Favored: asymmetric-split (Hero), Matrix Grid (KPI Cluster), Stacked Band, Asymmetric Split.
...
**Hero Monolith** — Opening anchor. Default treatment: asymmetric-split. Alts: the other two treatments below.
...
**Grouped Metric Cluster** — Parallel metrics, one claim. **Matrix Grid 2×2/2×3**: CSS `grid gap-1`, `bg-surface-l2 rounded-[16px] p-6 md:p-8`, no extra border/shadow; cell `h-full`, note `mt-auto`; hover `whileHover={{ y: -2 }}` 0.28s.
...
**Outro Reverent** — End of report. Typographic Field. `OutroSignature` component (§17) — twin hairlines + diamond rotate 0→45°. Optional — emit only with closing claim.
...
Ground consistent across chapters; vary rhythm through density, hairlines, accent position. Chapter markers: `01` `02` `03`. Editorial pack uses `ChapterBanner` (ShadBadge filled + hairline-banner divider) — no `ChapterStamp` SVG. Chapter opener: `ChapterBanner` every non-Hero chapter. Chapter divider for in-chapter rests, sparingly.
...
Composition — asymmetric-split: `grid-cols-1 lg:grid-cols-[1fr_auto] lg:gap-x-16 items-start`. Left: eyebrow → hero title → subtitle → focal block. Right: metadata masthead `hidden lg:flex flex-col items-end gap-10` — ISSUE/PAGES/AUDIT/AUTHOR mono uppercase.

Hero brand marks: focal numeral, delta indicator, shader breath. No signature line at bottom.
...
<motion.div initial="hidden" animate="visible" variants={heroVariants} className="relative z-20">
  <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] lg:gap-x-16 items-start">
    <div className="flex flex-col items-start text-left">
      ...
      <motion.h1 variants={fadeUp} className="max-w-[20ch] text-[64px] md:text-[140px] font-medium leading-[0.92] tracking-[-0.05em] text-[var(--foreground)]">{heroTitle}</motion.h1>
      <motion.p variants={fadeUp} className="max-w-[52ch] text-[15px] font-normal leading-[1.6] text-[var(--foreground-3)]">{subtitle}</motion.p>
      <motion.div variants={fadeUp} className="inline-flex items-baseline gap-1 whitespace-nowrap">
        ...
        <AnimateNumber style={{ color: "var(--primary)" }} transition={{ type: "spring", duration: 2.2, bounce: 0 }}>{number || 0}</AnimateNumber>
...
| Quote · Typographic Field | Quote text `font-normal` foreground | SVG L-shape + attribution | column break |
| Outro · Typographic Field | closing title Section Primary | `OutroSignature` + caveats | single stroke |
| Hero · asymmetric-split | focal Display Number `--primary` | metadata mono uppercase | scale jump |
...
Hero ember-wash CSS: `radial-gradient(ellipse 800px 600px at 50% 38%, var(--ember-wash), transparent 70%)`. Layer stack within Hero section (bottom→top): GrainGradient (z:10) · ember-wash (z:15) · **Hero text wrapper must be `relative z-20` or higher**. Outro mirrors at page-bottom-center, smaller.
  ...
  return (
    <div className="px-4 py-3" style={{
      ...
      <div className="text-[10px] uppercase mb-2" style={{
        ...
        letterSpacing: "0.16em", color: "var(--foreground-3)",
...
**ChapterBanner** — `--border-strong` hairline + `CHAPTER · NN` `text-[11px] md:text-[12px]` `tracking-[0.16em]` `--primary` + bilingual label `tracking-[0.12em]` `--foreground-3` + title `text-[34px] md:text-[44px] lg:text-[56px] font-medium tracking-[-0.03em]` `--foreground`. `whileInView` + `viewport={{ once: true, margin: "0px 0px -10% 0px" }}`, `duration: 1.2, ease: EASE.spring`.
