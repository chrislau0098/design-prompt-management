...
template_version: v0.5(R-90 festive-royal serif 扩展)
---
...
:root {
  ...
  --chart-1:       var(--primary);
  --chart-2:       oklch(0.78 0.13 var(--brand-hue));
  --chart-3:       oklch(0.66 0.1 var(--brand-hue));
  --chart-4:       oklch(0.54 0.08 var(--brand-hue));
  --chart-5:       oklch(0.44 0.06 var(--brand-hue));
...
**TasselDivider** — Vertical decorative element: 1px line 32px tall + small circle dot at end. Hero use only, max 1 instance.

```tsx
function TasselDivider() {
  return (
    <svg width="4" height="40" viewBox="0 0 4 40" aria-hidden style={{ display: "block" }}>
      <line x1="2" y1="0" x2="2" y2="32" stroke="var(--primary)" strokeOpacity="0.5" strokeWidth="1" />
      <circle cx="2" cy="37" r="3" fill="var(--primary)" fillOpacity="0.6" />
    </svg>
  );
}
```
