...
- Depth from Hero shader, chapter rhythm, typographic scale, and brand accent — not shadow-heavy cards. Borders whisper-thin; surfaces stack by ≤ one L step.
...
Shader policy: 1 WebGL instance per page (Hero only). Pauses to `speed: 0` off-viewport, returns `speed: 0.3` on re-enter.
...
| WebGL Hero | animated, viewport-paused | speed: 0.3 |
...
Continuous motion: Hero `MeshGradient` speed 0.3 in viewport, 0 out. SVG strokes one-shot. No infinite breathing, no scroll-linked transforms.
...
Ground consistent across chapters; vary rhythm through density, hairlines, accent position. Chapter markers: `01` `02` `03`, ChapterStamp at most 3× when optional. Chapter opener: `ChapterBanner` every non-Hero chapter. Chapter divider for in-chapter rests, sparingly.

## 13. Section Anatomy

### 13.1 Hero Monolith

Focal: primary-value Display Number, display stack `font-medium` `--primary`, inline unit `--primary-hl`, animated count-up. Background: page-level `var(--background)` ground + MeshGradient Hero anchor (Hero section internal, see §17, viewport-paused). 
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 pointer-events-none"
        ...
        <MeshGradient
          colors={['#F2F4F8', '#E0E7EF', '#1E3FB0', '#E0E7EF', '#F2F4F8']}
          distortion={ 0.15 }
          swirl={ 0.05 }
          grainOverlay={ 0 }
          opacity={ 0.18 }
          speed={!reducedMotion && inView ? 0.3 : 0}
...
**SpotlightGradient** — Hero overlay above shader, below text. CSS radial with slow drift, `s/cycle infinite`, paused via reduced-motion.
  ...
  return (
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 15 }} aria-hidden>
      <div style={{
        ...
        background: "",
        animation: "spotlight-drift s ease-in-out infinite alternate",
      }} />
    </div>
  );
}
...
**OutlinedPill** (Tags) — `inline-flex items-center px-3 py-1 rounded-full border` with `1px solid var(--border-strong)` background `var(--surface-l1)`. `text-[12px] font-medium` `--foreground-2`. No fill, no shadow — thin-border-inset only.

**Drawn-horizon SVG line** — Hero structural line `<motion.line>` `var(--primary)` 1px stroke, `pathLength: 0→1`, `delay 0.4s, duration 2.8s, ease: EASE.out`. Placed at Hero focal y-coordinate, full-bleed width.
