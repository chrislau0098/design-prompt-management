...
:root {
  ...
  --foreground:    oklch(0.140 0.008 232);
  --foreground-2:  oklch(0.420 0.008 232);
  --foreground-3:  oklch(0.620 0.008 232);
  --border:        oklch(0.140 0.008 232 / 0.10);
  --border-strong: oklch(0.140 0.008 232 / 0.22);
...
Font stacks: Primary Sans `'Helvetica Neue', 'IBM Plex Sans', 'Neue Haas Grotesk', 'Noto Sans SC', 'PingFang SC', sans-serif`; Display Number `'Helvetica Neue', 'IBM Plex Sans', 'Neue Haas Grotesk', sans-serif`; Mono Meta `'IBM Plex Mono', ui-monospace, monospace`. All sans — no serif, no italic.
...
Surface: flat ground from `var(--background)` token (no page-level shader). Hero uses a static CSS background layer (dot-grid / line-grid), scoped to Hero section — no WebGL anywhere on page. Panels lift through hairline + one-step L difference, never shadow stacks or backdrop-blur.

Borders: Whisper `1px solid var(--border)` · Strong `var(--border-strong)`. Shadow: none on cards.

Shader policy: page has **no WebGL layer**. Hero background relies on static CSS layer (dot-grid / line-grid) plus typographic scale alone — no shader breath, no opacity drift.





Chapter opener: `ChapterStamp` SVG **solid-fill** circle (40-56px, `fill="var(--primary)"`, no ring) + centered ALL-CAPS chapter title sans 700 + full-width `HairlineRule` 1px `--border-strong` below. No gradient, no dot, no banner kicker.
...
Engines: `motion/react` · `motion-plus/react` (AnimateNumber). No `@paper-design/shaders-react` — Hero uses static CSS layer.
...
| Hero background | static CSS dot-grid / line-grid layer | no motion |
...
Stagger: `staggerChildren: 0.10` from `"first"`. Reduce by 0.02 on mobile.
...
Continuous motion: none in Hero — background is static CSS. SVG strokes one-shot. No infinite breathing, no scroll-linked transforms.
...
**Outro Reverent** — End of report. Typographic Field. Closing claim Section Primary ALL-CAPS sans 700 + terminal full-width `HairlineRule` + optional final solid-fill `ChapterStamp`. No diamond, no ornament beyond hairline. Optional — emit only with closing claim.
...
Ground consistent across chapters; vary rhythm through density, hairlines, accent position. Chapter markers: `01` `02` `03`, ChapterStamp at most 3× when optional. Chapter opener: `ChapterStamp` SVG **solid-fill** circle + `HairlineRule` rule every non-Hero chapter (one per chapter, no exceptions). Chapter divider for in-chapter rests, sparingly.

## 13. Section Anatomy

### 13.1 Hero Monolith

Focal: primary-value Display Number, display stack `font-medium` `--primary`, inline unit `--primary-hl`, animated count-up. Background: page-level `var(--background)` ground + static CSS dot-grid layer (see §17, no WebGL). 

Composition — typographic-field: Centered type-only field. No chart, no figure — claim-line title carries the section.

Hero brand marks: focal numeral, delta indicator, static CSS background layer. ALL-CAPS Hero title carries the section over the static dot-grid.
...
```tsx
const heroVariants = { hidden: {}, visible: { transition: { staggerChildren: 0.10 } } };
...
| Outro · Typographic Field | closing title Section Primary | closing solid-fill `ChapterStamp` + `HairlineRule` + caveats | single stroke |
...
**Page ground** — Page background uses `var(--background)` token directly. Set via `style={{ background: "var(--background)" }}` on the App root `<div>`. The page has **no WebGL layer at all**. Hero background uses a static CSS layer (dot-grid / line-grid) — see Hero static background block below.
...
**App root structure** — Page background uses `var(--background)` flat token directly on the App root `<div>`. Do not wrap chapter sections in any semantic container (`<main>` / `<article>` / `<section>` wrapping all sections) — semantic wrappers create extra stacking contexts that conflict with absolute-positioned shaders inside Hero. The Hero static background layer lives inside Hero `<section className="relative">`; all other chapter sections are direct children of the App root `<div>` and inherit the flat `var(--background)` ground.

 Hero static CSS dot-grid layer (z:10) — single `<div className="hero-static-bg" aria-hidden />` per Hero. No motion. Text wrapper `relative z-20`.
...
**ChapterStamp** — SVG **solid-fill** circle, no border ring. Diameter 48–56px, `fill="var(--primary)"`, inside an ALL-CAPS sans 700 numeral `"01"` / `"02"` / `"03"` in `var(--background)` color. Helvetica / IBM Plex. No bilingual label below, no kicker — the stamp speaks alone.

```tsx
function ChapterStamp({ n }: { n: string }) {
  return (
    <svg width="52" height="52" viewBox="0 0 52 52" aria-hidden>
      <circle cx="26" cy="26" r="26" fill="var(--primary)" />
      <text x="26" y="34" textAnchor="middle"
        fontFamily="'Helvetica Neue', 'IBM Plex Sans', sans-serif"
        fontWeight="700" fontSize="22" fill="var(--background)">{n}</text>
    </svg>
  );
}
```

**HairlineRule** — Full-width 1px rule, no gradient, no dot, no fade. Stretches from container edge to container edge.
...
**SharpTag** — Replaces pill / `Badge` / `OutlinedPill`. **Sharp 0px corner radius** outline only. Thin border + sans 500 label.
...
| Archetype | Primary |
|---|---|
| Hero | `motion.div` stack + static CSS background layer (dot-grid / line-grid) |
...
| Outro | `motion.div` + closing solid-fill `ChapterStamp` + `HairlineRule` |
...
Use top-level hooks, typed props, `motion/react`, `motion-plus/react`, numeric easing arrays, `aria-hidden` for decorative visuals, semantic labels for readable values, `isAnimationActive={false}` on Recharts, and reduced-motion fallbacks for AnimateNumber, and off-screen continuous motion.
