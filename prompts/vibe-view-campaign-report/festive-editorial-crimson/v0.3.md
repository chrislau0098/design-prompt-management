...
changelog_note: "R-95 trim · ≤620 行 / 删 §2 重复 color rule prose + §6 重复 whileInView prose + §17 Tooltip 三段合并 + §10 Scenario/Voice 浓缩 + §4 / §13.1 / §11.3 verbose prose 合并 + 多处装饰空行;约束力 0 弱化"
---
...
Brand hue: `--brand-hue` is the only accent knob. Neutral tokens (backgrounds, surfaces, foregrounds, borders) fixed at hue 28.
...
Radius pill/tag none. Sharp panel `rounded-[0px]`; card chrome `rounded-[0px]`. KPI row 4–6 cells, vertical hairline column rule, Display Number + unit + caption each; `px-6 py-4` (first `pl-0`); stack below `md:` with horizontal hairline.
...
SVG `pathLength: 0→1` draw-in (no `d` morph). Continuous motion: Hero `GrainGradient` speed 1.3 in viewport, 0 out; SVG strokes one-shot; no infinite breathing, no scroll-linked transforms.
...
Curator's wall text: third-person observational, never `you`. Statements not CTAs (`Bookings closed at 142,300`, not `We booked 142,300!`). Eyebrows uppercase mono `tracking-[0.18em]`. Titles sentence case, no terminal period. Body full sentences, terminal punctuation. Numbers tabular, thousands separators, no decimal padding. Period strings `·` middle dot (U+00B7) single-spaced.
...
Composition — typographic-field: Centered type-only field. No chart, no figure — claim-line title carries the section. Alts: the two other treatments per §11.3.
...
- **`<Tooltip contentStyle>`**: required. `{{ background: "var(--surface-l2)", border: "1px solid var(--border-strong)", color: "var(--foreground)" }}` for inline; or `content={<ChartTooltipCard />}` for custom (festive-editorial uses ChartTooltipCard with white bg + black text).
...
**HairlineRule** — Full-width 1px rule, no gradient/dot/fade: `<hr style={{ border: 'none', borderTop: '1px solid var(--border-strong)', margin: 0 }} aria-hidden />`.
...
**Festive-Editorial discipline** — Light ivory ground. Hairline-only depth: no `box-shadow`, no `backdrop-blur`, no noise overlay, no SpotlightGradient, no OutroSignature, no double-bezel, no Dithering, no festive-royal SealStamp / GoldenHairline. Sans stack (Helvetica Neue / IBM Plex Sans / Inter / Noto Sans SC) throughout. 0px corner radius everywhere. Hero = GrainGradient red shader; Hero ≠ chapter (no ChapterNumeralLarge in Hero). Chapter opener = ChapterNumeralLarge + HairlineRule every non-Hero chapter. Tooltip = `<ChartTooltipCard />` (white bg + black text).
