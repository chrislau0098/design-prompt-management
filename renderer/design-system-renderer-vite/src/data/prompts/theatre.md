...
description: "A dark theatrical stage where numbers perform — quiet luxury via single accent, depth via linework, light, and motion."
...
:root {
  ...
  --primary-glow:  oklch(0.650 0.175 var(--brand-hue) / 0.18);
  --chart-1:       var(--primary);
  ...
  --quote-bg:      oklch(0.115 0.004 248);
...
Color rules:
- Focal numbers use `--primary` (L 0.650 C 0.175). Units, unit suffixes, percent signs, delta arrows, and delta values use `--primary-hl` (same hue, lighter L 0.720 C 0.160).
...
Brand hue: `--brand-hue` is the only accent knob. Accent tokens take hue from it; neutral tokens (backgrounds, surfaces, foregrounds, borders) fixed at hue 248; foreground prose shifts to hue 139 for measured offset against the neutral ground.
...
```css
/* system-fallback loading — primary Noto Sans SC Variable · display Geist Variable · mono Geist Mono */
```

Font stacks: Primary Sans `'Noto Sans SC Variable', 'PingFang SC', 'Source Han Sans SC', sans-serif`; Display Number `'Geist Variable', -apple-system, system-ui, sans-serif`; Mono Meta `'Geist Mono', ui-monospace, monospace`. 

Weight: `font-normal` (400) prose · `font-medium` (500) titles, Display Number, delta value, chapter marker, unit suffix. `font-semibold` (600) reserved for delta value and chapter marker numeral only. No `font-bold`/≥700.

| Level | lg | mobile | Weight |
|-------|----|--------|--------|
| Page Title | `text-[48px]` | `text-[28px]` | `font-medium` |
| Display Number | `text-[192px]` | `text-[80px]` | `font-medium` |
| Section Primary | `text-[136px]` | — | `font-medium` |
| Section Secondary | `text-[88px]` | — | `font-medium` |
| Section Tertiary | `text-[56px]` | — | `font-normal` |
...
Display Number `leading-[0.86]` `tracking-[-0.055em]`. Focal numbers use exact breakpoint classes (no `clamp()` — sub-pixel jitter degrades spring count-up). Floor `text-[10px]`; required reading `text-[14px]+` per §8.

Display Number anatomy: focal figure Display Number scale `--primary`;  `--primary` / `--primary-hl` mixed, baseline-aligned `gap-2`. Unit ~28–30% figure height; never same scale, never above baseline, never wrapped.
...
Radius: pill/tag: rounded-full. Sharp panel (table/chart): `rounded-[0px]`–`rounded-[6px]`. Card chrome: Double-Bezel (outer `p-1.5 ring-1 ring-white/5 rounded-[6px]` → inner `rounded-[4px] bg-surface-l2 p-6 md:p-8`).
...
Surface: flat ground from `var(--background)` token (no page-level shader). Hero shader is the page's only WebGL layer, scoped to Hero section. Panels lift through hairline + one-step L difference, with inset-glow shadow on Double-Bezel cards only.

Borders: Whisper `1px solid var(--border)` · Strong `var(--border-strong)`. Shadow: inset-glow on cards.

Shader policy: 1 WebGL instance per page (Hero only). Pauses to `speed: 0` off-viewport, returns `speed: 0.5` on re-enter.

Ambient ink: `--glow-ink` ~18% alpha tint overlay applied to Hero shader stack for stage-light coherence.



Chapter opener: `ChapterStamp` SVG circular badge — 1.5px `--primary` border ring (40×40), mono numeral centered `font-semibold` `--primary`, optional bilingual label below.

Chapter divider: `oklch(1 0 0 / 0.08) h-px`. In-chapter rest only.

Panel discipline: `rounded-[2px]`–`rounded-[6px]` chart frames, Double-Bezel card chrome. Each archetype one material logic.
...
| Number rolling | 1.0-2.4s | spring, no bounce |
| Curve pathLength | 2.4-3.2s | out |
| Spotlight drift | 7s/cycle | inOut, infinite |
| WebGL Hero | animated, viewport-paused | speed: 0.5 |
...
Continuous motion: Hero `MeshGradient` speed 0.5 in viewport, 0 out. SVG strokes one-shot. No infinite breathing, no scroll-linked transforms.

## 7. Iconography

`lucide-react`, 1.5px stroke, outline-only, `currentColor`, rem sizing, `gap-2` to text. Icons ≤ cap-height. Decorative: `aria-hidden`. Semantic: `role="img"` + `aria-label`. Custom SVG only for structural-only use cases. Unicode ▲ ▼ · — • allowed inline.
...
- Focal numerals in `--primary`; units and delta mix `--primary` / `--primary-hl` at baseline. Hero leads with confident sans medium title + paragraph subtitle.
...
- **Don't** use unauthorized accent palettes outside the brand hue. No heavy shadows, frosted glass, `backdrop-blur`, rim lighting, card-edge glow, or text glow except the documented Double-Bezel inset-glow.
- **Don't** use `rounded-xl`/`rounded-2xl`/`rounded-3xl`/blob cards. Sharp panels `rounded-[0px]`–`rounded-[6px]`; card chrome Double-Bezel only.
...
Copy register: 暗调戏剧舞台,Hermès 单橙强调. Chapter titles 5–14 CJK / 4–9 EN words, `font-medium` sans. Eyebrow tracking `0.14em`. Resist slogans, questions, ALL-CAPS.
...
**Grouped Metric Cluster** — Parallel metrics, one claim. **Matrix Grid 2×2/2×3**: CSS `grid gap-1`, `bg-surface-l2 Double-Bezel p-6 md:p-8`, no extra border/shadow beyond declared depth treatment; cell `h-full`, note `mt-auto`; hover `whileHover={{ y: -2 }}` 0.28s.
...
**Outro Reverent** — End of report. Typographic Field. Closing claim Section Primary + optional final `ChapterStamp` numeral. Optional — emit only with closing claim.

## 12. Composition

Default: Hero → evidence chapters → optional Outro. Section count follows data; stop before filler. This style favors 5–8 chapters, balanced cadence.
...
3. **Bold-move budget** — disruptive ≤ ceil(N × 0.25). This style runs medium.
4. **Break cadence** — ≥ 1 section uses Full-bleed, Offset, or Typographic Field. Hero + Outro satisfy.

### 12.2 Rules

Ground consistent across chapters; vary rhythm through density, hairlines, accent position. Chapter markers: `01` `02` `03`. ChapterStamp at most 3× when optional. Chapter opener: `ChapterStamp` SVG circular badge (border ring) optional per chapter. Chapter divider for in-chapter rests, sparingly.

## 13. Section Anatomy

### 13.1 Hero Monolith

Focal: primary-value Display Number, display stack `font-medium` `--primary`, inline unit `--primary-hl`, animated count-up. Background: page-level `var(--background)` ground + MeshGradient Hero anchor (Hero section internal, see §17, viewport-paused). Above the Hero shader, `--glow-ink` ambient tint.
...
<motion.div initial="hidden" animate="visible" variants={heroVariants} className="relative z-20">
  <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] lg:gap-x-16 items-start">
    <div className="flex flex-col items-start text-left">
      ...
      <motion.h1 variants={fadeUp} className="max-w-[28ch] text-[80px] md:text-[192px] font-medium leading-[0.86] tracking-[-0.055em] text-[var(--foreground)]">{heroTitle}</motion.h1>
...
| Outro · Typographic Field | closing title Section Primary | closing `ChapterStamp` (ring) + caveats | single stroke |
...
**Page ground** — Page background uses `var(--background)` token directly. Set via `style={{ background: "var(--background)" }}` on the App root `<div>`. The page has no page-level WebGL shader; the only WebGL layer is Hero `<MeshGradient>` (see below).
...
**MeshGradient** (@paper-design/shaders-react) — Hero anchor shader.
    ...
    <>
      ...
      <div className="fixed inset-0 pointer-events-none"
        ...
        <MeshGradient
          ...
          grainOverlay={ 0.12 }
          opacity={ 0.52 }
          speed={!reducedMotion && inView ? 0.5 : 0}
...
Hero glow-ink overlay: ambient inked tint above shader, below text. Layer stack within Hero section (bottom→top): MeshGradient (z:10) · glow-ink (z:15) · **Hero text wrapper must be `relative z-20` or higher**.
...
**ChapterStamp** — SVG circular badge: 1.5px `--primary` border ring (40×40), centered mono numeral `font-semibold` `--primary`. Optional bilingual label below, mono `tracking-[0.14em] --foreground-3`.
...
| Outro | `motion.div` + closing `ChapterStamp` (ring) |
